...
$proposal_page_url  = Workreap_Profile_Menu::workreap_profile_menu_link('proposals', $user_identity, true, 'listing',$project_id);
...
$workreap_attr['project_title']  = !empty($product) ? $product->get_name() : '';
...
if( !empty($proposal_type) && $proposal_type === 'milestone'){
    ...
    if( !empty($milestone) ){
        ...
        if( !empty($milestone_total) && $milestone_total == $earned_balance ){
            $workreap_attr['complete_option']         = 'yes';
        }
        ...
        $requested_milestone    = !empty($hired_milestone) ? array_merge($requested_milestone,$hired_milestone) : $requested_milestone;
...
<div class="wr-project-wrapper wr-activities-<?php echo esc_attr($project_type);?>">
    <div class="wr-project-box wr-employerproject">
        ...
        <div class="wr-price">
            ...
            <div class="wr-project-detail">
                <a href="<?php echo esc_url($proposal_page_url);?>" class="wr-btn-solid-lg"><?php esc_html_e('View all proposals','workreap');?></a>
            </div>
        </div>
    </div>
    <div class="wr-projectstatus">
        <?php if( !empty($no_of_freelancers) && $no_of_freelancers >1 && !empty($user_type) && $user_type === 'employers' ){ ?>
            <div class="wr-projectstatus_users">
                <?php workreap_get_template_part('dashboard/post-project/dashboard', 'hired-proposals',$workreap_attr);?>
            </div>
        <?php } ?>
        <div class="wr-projectsstatus">
            <?php workreap_get_template_part('dashboard/post-project/dashboard', 'project-freelancer-basic',$workreap_attr);?>
            <?php 
                if( !empty($proposal_type) && $proposal_type === 'milestone') {
                    workreap_get_template_part('dashboard/post-project/dashboard', 'project-milestone',$workreap_attr);
                } else if( !empty($project_type) && $project_type != 'fixed' ){
                    do_action( 'workreap_employer_project_basic_details', $proposal_id );
                }
            ?>
            <div class="wr-projectsactivity">
                <ul class="nav nav-tabs wr-nav-tabs" id="projectActivitiesTabs" role="tablist">
                    ...
                    <?php do_action( 'workreap_project_employer_avtivity_tabs',$workreap_attr );?>
                </ul>
                <div class="tab-content wr-project-type-content" id="projectActivitiesTabsContent">
                    ...
                    <?php 
                        ...
                        do_action( 'workreap_project_employer_avtivity_tab_content', $workreap_attr );
                    ?>
                </div>
            </div>
        </div>
