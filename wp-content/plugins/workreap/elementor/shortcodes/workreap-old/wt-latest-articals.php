...
if( !class_exists('Workreap_Latest_Articles') ){
	class Workreap_Latest_Articles extends Widget_Base {
		...
		public function get_title() {
			return esc_html__( 'Latest Articles', 'workreap' );
		}
		...
		protected function register_controls() {
			...
			$this->add_control(
				...
				[
					...
					'label' 		=> esc_html__('Posts ID\'s', 'workreap'),
        			'description' 	=> esc_html__('Add Posts ID\'s with comma(,) separated e.g(15,21). Leave it empty to show latest posts.', 'workreap'),
				]
			);
			...
			$this->add_control(
				...
				[
					...
					'step' 			=> 1,
					'default' 		=> 10,
					'label' 		=> esc_html__('Number of latests posts', 'workreap'),
        			'description' 	=> esc_html__('Add Number of latests posts. It\'s only working if  Posts ID\'s field is null.', 'workreap'),
				]
			);
										...
										<div class="wt-articles">
											...
											<div class="wt-articlecontents">
												<div class="wt-title">
													...
													<span class="wt-datetime"><i class="wr-calendar"></i><?php echo date_i18n(get_option('date_format'), strtotime(get_the_date('Y-m-d', $post->ID))); ?></span>
												</div>
												<ul class="wt-moreoptions">
													<li><a href="<?php echo esc_url( get_comments_link( $post->ID ) ); ?> "><i class="wr-comment"></i><?php echo  do_shortcode($write_comments); ?></a></li>
													<li><i class="wr-eye"></i> <?php echo intval($post_view_count); ?>&nbsp;<?php esc_html_e('View(s)','workreap');?></li>
												</ul>
											</div>
										</div>
									</div>
