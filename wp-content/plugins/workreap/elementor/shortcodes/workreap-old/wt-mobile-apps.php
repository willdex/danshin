...
if( !class_exists('Workreap_APPS_Options') ){
	class Workreap_APPS_Options extends Widget_Base {
		...
		public function get_title() {
			return esc_html__( 'Mobile APPS V1', 'workreap' );
		}
		...
		protected function register_controls() {
			...
			$this->add_control(
				...
				[
					...
					'description'   => esc_html__( 'Add Google Play store image.', 'workreap' ),
				]
			);
			...
			$this->add_control(
				...
				[
        			...
        			'description' 	=> esc_html__('Add Google Play store link. Leave it empty to hide.', 'workreap'),
				]
			);
			...
			$this->add_control(
				...
				[
					...
					'description'   => esc_html__( 'Add App store image.', 'workreap' ),
				]
			);
			$this->add_control(
				'button_text_ios',
			...
			$this->add_control(
				...
				[
        			...
        			'description' 	=> esc_html__('Add App store link. Leave it empty to hide.', 'workreap'),
				]
			);
							...
							<?php if( ( !empty( $google_image ) && !empty( $play_store_url ) ) || ( !empty( $app_store_image ) && !empty( $app_store_url ) ) ) {?>
								<ul class="wt-appicons">
									<?php if( !empty( $google_image ) && !empty( $play_store_url ) ) {?>
										<li><a class="<?php echo esc_attr($button_text_class);?>" href="<?php echo esc_url( $play_store_url );?>"><?php if(!empty($button_text)){ echo esc_attr($button_text);}else{?><img src="<?php echo esc_url( $google_image );?>" alt="<?php esc_attr_e('Play store','workreap');?>"><?php }?></a></li>
									<?php }?>
									<?php if( !empty( $app_store_image ) && !empty( $app_store_url ) ) {?>
										<li><a class="<?php echo esc_attr($button_text_ios_class);?>" href="<?php echo esc_url( $app_store_url );?>"><?php if(!empty($button_text_ios)){ echo esc_attr($button_text_ios);}else{?><img src="<?php echo esc_url( $app_store_image );?>" alt="<?php esc_attr_e('App store','workreap');?>"><?php }?></a></li>
									<?php }?>
								</ul>
							<?php }?>
						</div>
	...
	Plugin::instance()->widgets_manager->register_widget_type( new Workreap_APPS_Options ); 
}