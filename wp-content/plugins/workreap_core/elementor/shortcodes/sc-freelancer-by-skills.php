...
if( !class_exists('Workreap_Freelancer_By_Skills') ){
	class Workreap_Freelancer_By_Skills extends Widget_Base {
		...
		public function get_title() {
			return esc_html__( 'Freelancers by skills', 'workreap_core' );
		}
		...
		protected function register_controls() {
			$categories	= elementor_get_taxonomies('projects', 'skills', 0);
							...
							<ul class="wt-categoryvtwo wt-categoryvthree">
								<?php foreach( $categories as $key => $cat_id ) { 
									...
									if( function_exists( 'fw_get_db_term_option' ) ) {
										$icon          = fw_get_db_term_option($cat_id, 'skills');
										$category_icon = !empty($icon['skills_icon']) ? $icon['skills_icon'] : array();
									}
									?>
									<li>
