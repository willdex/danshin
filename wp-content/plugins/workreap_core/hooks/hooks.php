...
if (!function_exists('workreap_ajax_login')) {
    ...
    function workreap_ajax_login() { 
        ...
        $user_array = array();
		$json		= array();
        $user_array['user_login'] 			= esc_html($_POST['username']);
        $user_array['user_password'] 		= ($_POST['password']);
        ...
        if (isset($_POST['rememberme'])) {
            $remember = esc_html($_POST['rememberme']);
        } else {
            $remember = '';
        }
								   ...
								   || ( isset($enable_linkedin_connect) && $enable_linkedin_connect === 'enable' ) 
									...
									<div class="wt-loginicon">
										...
										<ul>
											...
											<?php if (  isset($enable_linkedin_connect) && $enable_linkedin_connect === 'enable' ) {?><li class="wt-linkedin"><a class="sp-linkedin-connect" href="#" onclick="event_preventDefault(event);"><i class="fa fa-linkedin"></i></a></li><?php } ?>
											<?php if (  isset($enable_google_connect) && $enable_google_connect === 'enable' ) {?><li><a href="#" onclick="event_preventDefault(event);" id="wt-gconnect-reg"></a></li><?php } ?>
										</ul>
									</div>
								<?php } ?>
							</div>
	...
	function demo_login_details($classes=''){
		if( isset( $_SERVER["SERVER_NAME"] ) && 
		   ( $_SERVER["SERVER_NAME"] === 'amentotech.com' 
			 ...
			 ||  $_SERVER["SERVER_NAME"] === 'demos.codingeasel.com'
		   )
		  ){
			?>
...
if( !function_exists( 'workreap_login_single_step' ) ){
	function workreap_login_single_step($type=''){
		...
		if(!empty($type) && $type === 'shortcode'){
			$columnClass	= 'col-sm-12 col-md-10 col-lg-5 wt-loginshortcodes';
		}
								   ...
								   || ( isset($enable_linkedin_connect) && $enable_linkedin_connect === 'enable' ) 
									...
									<div class="wt-loginicon">
										...
										<ul>
											...
											<?php if (  isset($enable_facebook_connect) && $enable_facebook_connect === 'enable' ) {?><li class="wt-facebook"><a href="#" onclick="event_preventDefault(event);" class="sp-fb-connect"><i class="fa fa-facebook-f"></i></a></li><?php } ?> 
											<?php if (  isset($enable_linkedin_connect) && $enable_linkedin_connect === 'enable' ) {?><li class="wt-linkedin"><a class="sp-linkedin-connect" href="#" onclick="event_preventDefault(event);"><i class="fa fa-linkedin"></i></a></li><?php } ?>
											<?php if (  isset($enable_google_connect) && $enable_google_connect === 'enable' ) {?><li class="wt-google-signin"><a href="#" onclick="event_preventDefault(event);"  id="wt-gconnect"></a></li><?php } ?>
										</ul>
									</div>
								<?php } ?>
							</div>
					   ...
					   || ( isset($enable_linkedin_connect) && $enable_linkedin_connect === 'enable' ) 
						...
						<ul class="wt-socialicons wt-iconwithtext">
							<?php if (  isset($enable_facebook_connect) && $enable_facebook_connect === 'enable' ) {?>
								<li class="wt-facebook"><a class="sp-fb-connect" href="#" onclick="event_preventDefault(event);"><i class="fa fa-facebook-f"></i></a></li>
							<?php }?>
							...
							<?php if (  isset($enable_google_connect) && $enable_google_connect === 'enable' ) {?>
								<li><a id="wt-gconnect-reg" href="#" onclick="event_preventDefault(event);"></a></li>
							<?php }?>
						</ul>
					<?php }?>
...
if( !function_exists( 'workreap_process_registration_step_two' ) ){
	function workreap_process_registration_step_two(){
        ...
        foreach ( $validations as $key => $value ) {
            ...
            if( $key == 'user_type'){
            	if( $_POST['user_type'] == 'company' ){
            		$employees  = !empty( $_POST['employees'] ) ? esc_html( $_POST['employees'] ) : '';
            		$department = !empty( $_POST['department'] ) ? esc_html( $_POST['department'] ) : '';
		...
		if (function_exists('fw_get_db_settings_option')) {
       	...
       	$password  	= !empty( $_POST['password'] ) ? esc_html( $_POST['password'] ) : '';
       	$user_type 	= !empty( $_POST['user_type'] ) ? esc_html( $_POST['user_type'] ) : '';
       	$department = !empty( $_POST['department'] ) ? esc_html( $_POST['department'] ) : '';
       	$employees  = !empty( $_POST['employees'] ) ? esc_html( $_POST['employees'] ) : '';
		$termsconditions  = !empty( $_POST['termsconditions'] ) ? esc_html( $_POST['termsconditions'] ) : '';
		...
		$user_data	= isset($_SESSION['user_data']) ? $_SESSION['user_data'] : array();
       	...
       	$username 	= !empty( $user_data['register']['username'] ) ? esc_html( $user_data['register']['username'] ) : '';
		$first_name = !empty( $user_data['register']['first_name'] ) ? esc_html( $user_data['register']['first_name'] ) : '';
       	$last_name 	= !empty( $user_data['register']['last_name'] ) ? esc_html( $user_data['register']['last_name'] ) : '';
       	$gender 	= !empty( $user_data['register']['gender'] ) ? esc_html( $user_data['register']['gender'] ) : '';
       	$email 		= !empty( $user_data['register']['email'] ) ? esc_html( $user_data['register']['email'] ) : '';
        ...
        if ( is_wp_error( $user_identity ) ) {
        	...
        	global $wpdb;
            wp_update_user( array('ID' => esc_sql( $user_identity ), 'role' => esc_sql( $db_user_role ), 'user_status' => 0 ) );
            ...
            if( !is_wp_error( $post_id ) ) {
				...
				if( $user_type === 'employer' && !empty($employer_package_id) ) {
					workreap_update_pakage_data( $employer_package_id ,$user_identity,'','employer' );
					$role_type		= esc_html__('Employer','workreap_core');
				} else if( $user_type === 'freelancer' && !empty($freelancer_package_id) ) {
					workreap_update_pakage_data( $freelancer_package_id ,$user_identity,'','freelancer' );
					$role_type		= esc_html__('Freelancer','workreap_core');
				}
...
if( !function_exists( 'workreap_process_social_registration_step_two' ) ){
	function workreap_process_social_registration_step_two(){
		...
		if( !is_wp_error( $post_id ) ) {
			...
			$wpdb->update(
					$wpdb->prefix . 'users', array('user_status' => 0), array('ID' => intval($user_identity))
			);
			...
			if( !empty( $switch_account ) && $switch_account == 'no' ){
				...
				if( $user_type === 'employer' && !empty($employer_package_id) ) {
					workreap_update_pakage_data( $employer_package_id ,$current_user->ID,'',$user_type );
				} else if( $user_type === 'freelancer' && !empty($freelancer_package_id) ) {
					workreap_update_pakage_data( $freelancer_package_id ,$current_user->ID,'',$user_type );
				}
			}
		...
		if( isset( $type ) && $type === 'project' ){
			...
			if (class_exists('Workreap_Published')) {
				if (class_exists('Workreap_Published')) {
					...
					$push['%name%']	= get_the_title($freelancer_profile_id);
					$push['%freelancer_name%']	= get_the_title($freelancer_profile_id);
					$push['%freelancer_link%']	= get_permalink($freelancer_profile_id);
...
if( !function_exists( 'workreap_print_login_form' ) ){
	...
	function workreap_print_login_form(){
		...
		$login_page_url			= !empty($enable_login_register['enable']['login_page'][0]) ? $enable_login_register['enable']['login_page'][0] : '';
			...
			<div class="wt-loginarea">
				<div class="wt-loginoption">
					<?php if( !empty( $header_type['gadget'] ) 
						...
						<div class="wt-loginoption wt-loginoptionvtwo">
							...
							<?php } else if( !empty( $enable_login_register['enable']['login_signup_type'] ) && $enable_login_register['enable']['login_signup_type'] === 'single_step' ){?>
								<a href="#" onclick="event_preventDefault(event);"  data-toggle="modal" data-target="#loginpopup" class="wt-btn"><i class="fa fa-sign-in"></i>&nbsp;<?php esc_html_e('Sign In','workreap_core');?></a>
							<?php } else if( !empty( $enable_login_register['enable']['login_signup_type'] ) && $enable_login_register['enable']['login_signup_type'] === 'pages' && !empty($login_page_url)){?>
								<a href="<?php echo esc_url(get_the_permalink( $login_page_url ));?>"  class="wt-btn"><i class="fa fa-sign-in"></i>&nbsp;<?php esc_html_e('Sign In','workreap_core');?></a>
							<?php } else {?>
					...
					<?php }else {?>
						...
						<?php }?>
					<?php }?>
					<?php if( !empty( $enable_login_register['enable']['login_signup_type'] ) && $enable_login_register['enable']['login_signup_type'] === 'pages' && !empty($login_page_url)){?>
							...
							<?php 
								...
								<div class="wt-joinnowholder">
									<ul class="wt-socialicons wt-iconwithtext">
										...
										<?php if (  isset($enable_linkedin_connect) && $enable_linkedin_connect === 'enable' ) {do_action('workreap_linkedin_login_button');}?>
										<?php if (  isset($enable_google_connect) && $enable_google_connect === 'enable' ) {?>
											<li><a id="wt-gconnect" href="#" onclick="event_preventDefault(event);"></a></li>
										<?php }?>
									</ul>
								</div>
							<?php }?>
    ...
    function workreap_save_project_meta_data($post_id) {
		...
		if ( get_post_type() === 'freelancers' ) {
			...
			if(!empty($freelancer_price_option) && $freelancer_price_option === 'enable' ){
				$min_price   = !empty($_POST['fw_options']['min_price'] ) ? esc_html( $_POST['fw_options']['min_price'] ) : '';
				$max_price   = !empty($_POST['fw_options']['max_price'] ) ? esc_html( $_POST['fw_options']['max_price'] ) : '';
...
if (!function_exists('workreap_create_social_users')) {
	...
	function workreap_create_social_users($type,$user) {
		...
		if ( !is_wp_error( $ID ) ) {
			global $wpdb;
			$db_user_role = '';
			wp_update_user( array('ID' => esc_sql( $ID ), 'role' => 'subscriber', 'user_status' => 0 ) );
...
if (!function_exists('workreap_do_upload_social_user_avatar')) {
	...
	function workreap_do_upload_social_user_avatar($user,$type,$user_id) {
		$filename	= !empty($user['id']) ? $user['id'].'.jpg' : rand(1,9999);
		$size_type  = 'avatar';
		$uploaddir 	= wp_upload_dir();
		$uploadfile = $uploaddir['path'] . '/' .$filename;

		if( isset( $type ) && $type === 'facebook' && !empty($user['id']) ){
			$url	= 'https://graph.facebook.com/'.$user['id'].'/picture?width=600';
		} else{
			$url	= !empty($user['picture']) ? $user['picture'] : '';
		}
...
if( !function_exists( 'workreap_single_step_registration' ) ){
	function workreap_single_step_registration(){
		...
		if(!empty($_POST['termsconditions'])){
       	...
       	//Get Data
		$location   = !empty( $_POST['location'] ) ? esc_html( $_POST['location'] ) : '';
		$first_name = !empty( $_POST['first_name'] ) ? esc_html( $_POST['first_name'] ) : '';
		$last_name  = !empty( $_POST['last_name'] ) ? esc_html( $_POST['last_name'] ) : '';
		$email   	= !empty( $_POST['email'] ) ? esc_html( $_POST['email'] ) : '';
		$username   = !empty( $_POST['username'] ) ? esc_html( $_POST['username'] ) : $email;
       	$password  	= !empty( $_POST['password'] ) ? esc_html( $_POST['password'] ) : '';
       	$user_type 	= !empty( $_POST['user_type'] ) ? esc_html( $_POST['user_type'] ) : '';
		$termsconditions 	= !empty( $_POST['termsconditions'] ) ? esc_html( $_POST['termsconditions'] ) : '';
