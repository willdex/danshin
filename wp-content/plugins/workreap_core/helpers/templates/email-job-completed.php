...
if (!class_exists('WorkreapJobCompleted')) {
    ...
    class WorkreapJobCompleted extends Workreap_Email_helper{
		...
		/**
		 * @Send admin job completed email
		 *
		 * @since 1.0.0
		 */
		public function send_job_completed_email_admin($params = '') {
			extract($params);
			$subject_default = esc_html__('Job Completed', 'workreap_core');
			...
			if (function_exists('fw_get_db_settings_option')) {
                $email_to 		= fw_get_db_settings_option('admin_job_complete_email');
				$subject 		= fw_get_db_settings_option('admin_job_complete_subject');
				$email_content 	= fw_get_db_settings_option('admin_job_complete_content');
			}

			//Verify either send or not
			$is_send = $this->send_email_verification('admin_job_complete_content_disable');
			if($is_send){return;}
		 ...
		 * @since 1.0.0
		 */
		public function send_job_completed_email_freelancer($params = '') {
			extract($params);
			$subject_default = esc_html__('Job Completed', 'workreap_core');
			$email_default = 'Hello %freelancer_name%
								Congratulation! You have complete your job.

								<a href=" %employer_link%">%employer_name%</a> has confirmed that the following project (<a href="%project_link%">%project_title%</a>) has been completed completed.
			...
			if (function_exists('fw_get_db_settings_option')) {
				$subject = fw_get_db_settings_option('frl_job_complete_subject');
				$email_content = fw_get_db_settings_option('frl_job_complete_content');
			}

			//Verify either send or not
			$is_send = $this->send_email_verification('frl_job_complete_content_disable');
			if($is_send){return;}
