...
if(!empty($order->get_items())){
	foreach ( $order->get_items() as $item_id => $item ) {
		...
		//Tax
		if( function_exists('wmc_revert_price') ){
			$tax	= workreap_price_format(wmc_revert_price($processing_fee,$order->get_currency()),'return');
		}else{
			$tax	= workreap_price_format($processing_fee,'return');
		}
		...
		$html	.= '<tr>
						...
						<td data-label="'. esc_html__('Taxes', 'workreap').'"><span>'. $tax .'</span></td>
...
if (!empty($main_logo['url'])) {
	...
	$logo = WorkreapCoreURI. '/images/logo.png';
}
